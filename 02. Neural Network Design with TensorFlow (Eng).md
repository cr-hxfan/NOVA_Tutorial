...
1. To understand the structure and feature of VGG, and the principle of the mainstram objet detection framework SSD
...
- One key feature of VGG is having a small filer size. VGG cascades multiple 3x3 filters to realise the reception field of a larger filter of size 5x5 or 7x7. This can effectively reduce the number of parameters, while provide more non-linear mapping, improving the ability of the network to fit the data. 



- Another obvious feature of VGG is more channels. The first layer of VGG has 64 channels, and the number of channels doubles every layer to a maximum of 512. As the number of channel increases, more information can be extracted



VGGNet explores the relation between the depth of the convolutional neural network and its performance. It successfully builds a 16- 19 layer CNN, proving increasing the depth can improve the final performance to a certain extent. It greatly reduces the error rate and can be generalised to other image data. Therefore, VGG is often used for image feature extraction, being the backbone network of other deep learning models.
...
Two-stage detection is split into 2 steps. First, the model proposes a set of regions that may contain the object (region proposal). Then, extract the feature of the candidate regions for classification and a more accurate location (regression).
...
1. From line 440 of `/lab_02/nets/ssd_KYnet_v3_6b.py`, use `slim` to implement the network structure shown above
2. Name the corresponding output layer correctly as required
3. From line 536 of `/lab_02/nets/ssd_KYnet_v3_6b.py`, define the parameters of convolution and pooling of the model
...
- `export_inference_graph.py`：Removing checkpoints that are irrelevant to evaluation, keeping the core computation checkpoints
...
cp ./test_release.json ./logs_test/image.json

cp ./test_release.INFO ./logs_test/image.INFO
...
3）After training the model, we can use `post_gen.py` to export the default parameters of the anchors of the model. This is to reduce the complexity of the model：For real-life application, evaluation can be speeded up by only running core computation on the accelerator, and use CPU for parts that are not derivation related. 
