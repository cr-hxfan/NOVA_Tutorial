...
This experiment briefly introduces Tensorflow and Convolutional Neural Network (CNN), starting from the basic elements of CNN, to the realization of computation in Tensorflow, and finally building a simple network. 

> Note: part of this experiment is recommended  to do in docker.
...
- [3. Exercise](#3-Exercise)
    ...
    - [3.1 Model Training and Validation](#31-Model-Training-and-Valiadation)
...
2. To learn and apply the relevant API in Tensorflow



3. To understand the conversion of the standard format `TFrecord` of Tensorflow, and common data pre-processing methods
...
**Data Flow Graph** is composed of **Nodes** 和 **Edges**.

- **Node** represents the operations (**OP**) to the data. It can also be used to represent the source or destination of the data

- **Edge** represents the connection of the nodes



The data transferred on the **Edge** is called **Tensor**. For a neural network that works with images, the common format of the image data is of the form: `batch*height*width*channel`。

- `batch` represents the sample size for each iteration

- `height` and `weight` represents the size of the sample

- `channel` represents the number of channels of the input image or feature map. For example, a coloured image has 3 channels (R, G, B)
...
The following graph illustrates the working principle of Tensorflow.
...
For image processing, convolution is an effective way for feature extraction. It aims at extracting and amplifying the feature of the object. The result of an image after convolution is called **feature map**.



In order to apply convolution, we need to confirm the size of **filter** and **step**. Depending on the requirement, sometimes we have to decide if **padding** is needed.
...
We use the most common 2-dimensional convolution API `tf.nn.con2d` as an example, the official documentation can be found in the following [link](https://tensorflow.google.cn/api_docs/python/tf/nn/conv2d).
...
Take max pooling as an example, the corresponding API in Tensorflow is `tf.nn.max_pool`, the official documentation can be found in the following [link](https://tensorflow.google.cn/api_docs/python/tf/nn/max_pool).
...
The API for the activation function ReLU in Tensorflow is `tf.nn.relu`. The official documentation can be found in the following [link](https://tensorflow.google.cn/api_docs/python/tf/nn/relu).
...
[CIFAR-01](http://www.cs.toronto.edu/~kriz/cifar.html)is a classic dataset for computer vision. It includes 60000 32x32 coloured images of 10 different types of objects, with 50000 images for training and 10000 for testing.  
...
Every image is stored in a flattened form (a 32x23 image with 3 channels is flattened to a list of length 3072). The data type is uint8. The first 1024 data in the list stores the red channel, the next 1024 data stores the green channel and the last 1024 stores the blue channel. 
...
As we used the `slim` module in Tensorflow, the layer implementation is different from 1.3 in this tutorial.
...
`slim` also provides `slim.avg_pool2d` API，uses can check the official document for the source code.
...
`tf.image` in Tensorflow also provides other API for image processing. Further details can be found in the this [link](https://www.tensorflow.org/api_docs/python/tf/image).
...
- `model_type` is for model selection. Here, we provide 2 predefined model structure, `slim` and `large`. The `large` model is to be defined by the user in the next experiment
